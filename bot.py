...
async def on_message(message):
    # Não deixa o bot apagar as próprias mensagens
    if message.author == bot.user:
        return

    # ID do canal que você quer que aceite APENAS imagens
    ID_CANAL_MIDIA = 1481092037993300018  # Substitua pelo seu ID

    if message.channel.id == ID_CANAL_MIDIA:
        # Verifica se a mensagem NÃO tem anexos
        if not message.attachments:
            await message.delete()
            # Opcional: Avisar o usuário no privado ou no chat (e apagar o aviso depois)
            await message.channel.send(f"{message.author.mention}, este canal é apenas para imagens!", delete_after=5)

    # Importante para os comandos continuarem funcionando
